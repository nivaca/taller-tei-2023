...
% \NewDocumentCommand \TFrameImg { o m }
% {%
% \begingroup
% \setbeamercolor{background canvas}{bg=white}%
% \setbeamertemplate{navigation symbols}{}%
...
%     \node at (current page.center){\includegraphics[width=\paperwidth,height=\paperheight, keepaspectratio]{img/#2}};
%     \IfValueTF {#1}
%     {\node[anchor=north east, fill=black!10] at (current page.north east){{\Azul\scriptsize#1}};}{}
%   \end{tikzpicture}
% \end{frame}%
% \endgroup%
% }
...
% \tcbset{arc=2mm, colbacktitle=MidnightBlue, fonttitle=\bfseries, opacityfill=0.3, boxrule=.5pt, colback=Gainsboro, size=fbox}
...
\begin{frame}[fragile]
  ...
  \begin{enumerate}
    ...
    \item \rojoit{Declaración}: está al principio de todo documento \XML, identificándolo como tal: \mintinline{xml}|<?xml version="1.0" encoding="UTF-8"?>|
  \end{enumerate}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item \mintinline{xml}|<note>|: indica que el texto marcado es una nota (\texttt{@type} puede indicar si es marginal, a pie de página, etc.)
